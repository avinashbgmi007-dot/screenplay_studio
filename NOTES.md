...
**Also:** §11's quoted `app.js` numbers were stale (`+944/-392`; the sweep commit moved it to `+985/-410`). The box's net-negative half was then ruled on: the writer took the recommendation, so §11 box 2 is retitled "the retired surfaces stay retired", ticked, and the line-count clause struck with the reason recorded inline. Plan Deviation 3 is updated to match. All 9 §11 boxes now carry a named gate and none is held open.
...
- **2026-09-23 — ONE DESK, ONE LEDGER / P3 gate: spec §11 walked, the plan is fully ticked, and one box was reported NOT met (since amended).**
...
**1. The escalation pin was invisible.** `discussWithDoctor` pinned the finding into `pendingQuote`, but that card renders only inside Sameer's composer, which lives in the room drawer while the doctor's lens is open. The writer escalated a finding to Dr. Sushruta and saw no trace of it on the surface they were reading; the send still carried the quote (`webapp_server` path reads `pendingQuote` for the consultant), so no other gate could notice. `renderQuoteCard` now paints both chat hosts (`#quote-card`, new `#fv-consult-quote-card`) — only one is ever on screen, so there is no second visible card to explain — and `#fv-consult-composer` gained `flex-wrap: wrap` because `.quote-card` is `flex: 1 0 100%; order: -1` and that composer is a row.
**2. A green gate that measured the wrong thing.** The width suite first asserted the pin with `getBoundingClientRect().width > 0` anywhere in the document — true for a card parked in a CLOSED drawer (Playwright-visible and DOM-visible are not the same claim). It now asks for the card inside the dock's un-hidden lens. Without that tightening the fix above would have had a passing test and a broken feature, exactly like the pre-P0 Problem Board.
**3. Dead CSS from a retired surface, found by grep and pinned.** `#feedback-view.fv-maximized` rules survived P0.1 in `style.css` with no host element and no JS ever setting the class; deleted, and `test_app_symbol_integrity.py` now fails if `#feedback-view` returns to the sheet, not just to the markup and JS.
**Box 2 was NOT met, on purpose — and is now amended (see the entry above).** `app.js` is +985/-410 against `main`, not net-negative: every deletion bought a surface the same spec asks for (persisted collapsible sections, the one-rendering dedupe, the convergence line, the stage ladder, `/quickcheck`, the rule popover). The retired-surfaces half is met and pinned in three files. Amending the box was a product call, so it was reported rather than ticked at gate time; the writer then ruled for the amendment.
**A store hazard worth knowing before you copy a project anywhere:** `ProjectManifest.from_dict` keeps the `project_dir` recorded at analysis time, so `ProjectManifest.load(some_dir)` returns a manifest that resolves every store back to the ORIGINAL directory. That is why `tests/_p1_visual_gate.py` (which `copytree`s `studio_projects/gun_pen_2`) reads the repo's own fixture rather than its copy, and why the arc line cannot be photographed from that copy — `pass_history.json` written into the copy is invisible to it. Pre-existing, harmless while `studio_projects/` is git-ignored and the gate only reads, and out of this plan's scope; the arc's dawn coverage went into `tests/e2e_browser_pass_arc.py` (2 new legs: the line still reads `Pass 2` at dawn, and its colours MOVE with the theme, so a hard-coded literal fails by construction).
**Plan state:** every box in `docs/superpowers/plans/2026-09-22-one-feedback-room.md` is now ticked; P0.1-P0.4, the P0 gate, P1.6 and P1.7 were retro-ticked with a note saying which commits and which live gates back each one. Task 21 carries 4 Deviations, including the stale `matchQuoteInScene` (P2.19 folded it into `inkMatch`).
**Still open after this gate (not blockers, all named):** ~40 orphan `.fv-*` selectors left in the sheet → **closed**, 43 dead ones removed and the family pinned by name in the entry above; `qoder/update` → **pushed** 2026-09-23 as a new remote branch (`origin/qoder/update` = `4143945`, 34 commits; the old credential-path blocker is gone — `GIT_TERMINAL_PROMPT=0 git push -u` worked first try). `main` is NOT moved: local `main` (`0d12d56`) is still 15 ahead of `origin/main`, and those commits reach the remote only under `qoder/update` until someone opens the PR or fast-forwards `main`; spec §2.2's doctor-gating orphan needs a product decision.

- **2026-09-23 — BROWSER SWEEP: every surface walked live at desktop width; it found one buried affordance, and that is fixed.**
*(the last requirement of the standing brief: "e2e testing, also browser level, each and every functionality")*
Method: a live studio (`--port 8512 --no-token`, throwaway projects dir) driven by real clicks at 1440×900 over a **36-finding project with 10 verified quotes** (`studio_projects/gun_pen_2` copied, with `project_dir` rewritten in the copy's `project.json` so the store hazard above cannot route its writes back to the repo fixture). 24 checks, all green, zero JS errors: manuscript ≥50% with the rail open (1394/1440); the four draft exports each **download** (fountain 6 KB, fdx 15 KB, txt 6 KB, backup 87 KB); rail scene outline + characters; the Stash & Notes lens holding both shelves; drag-select → 📥 Stash this actually persisting (2→3) and offering "Note this line"; all four dock lenses owning the room at 380 px with the paper at 1014 px; Sameer answering in the lens; 🩺 opening the consult **with the finding pinned and visible**; the doctor answering; the fix loop walking `n`/`p` and leaving on `esc`; `/quickcheck` answering 4 findings with `provisional: true` and no model; revision view and beat board opening (the board's reordered export carrying a real href); the settings modal's local/remote choice **refusing** `http://203.0.113.9:8080` with "Refusing to point the model server at…" and leaving the status line untouched; idea mode raising the canvas and dropping the manuscript+dock, and opening a project putting them back.
**What was actually wrong (the sweep's one real find):** the report export. `#report-export-btn` lives in the legacy Feedback drawer, and since P0.1 the desk's Feedback button routes to the **dock ledger** when a project is open — so the only gesture that ever showed that drawer was the selection popup's "Ask consultant". The export worked (200, 34 KB) but a writer could not find it: `tests/e2e_browser_export_flush.py` had quietly been reaching it with `page.evaluate("openFeedbackRoom()")` and a comment admitting the routing change. Fix: the overflow menu — where the other four exports already live — now offers "Export the report .md", painted by a new `paintReportExport()` that both hosts call, so the URL exists in exactly one place (`renderReportPanel`'s own copy-paste of it is gone). Gated RED-first with 3 new legs in that suite (visible in the menu → href targets `/report/export` → clicking downloads `Seed_Export-report.md`); the RED proof was run by disabling both call sites and watching them fail.
**Three things I first called failures and were not** (recorded so the next session doesn't re-chase them): the 54 `.scene-notes` cards that ignore the ONE filter are all `.finding-pin` — the writer's own pinned notes, deliberately not filtered; `--paper` looked frozen at dawn because the dawn block scopes tokens to `html body.dawn`, so reading them on `:root` proves nothing; and left-clipped text in `browser-use` screenshots at dpr 1.5 is a capture artifact — the DOM had every element at x ≥ 13 with no scroll and no overflow.
**Gates after the fix:** `pytest tests/` 1661 passed / 3 skipped; `run_browser_suites.py` 40 suites → 39 passed, 0 failed, 1 skipped (`gun_pen_audit` wants a live llama-server); `node --check` clean; `ruff` clean.
...
**What shipped:** `screenplay_studio/pass_history.py` — append-only `pass_history.json`, one entry per completed or partial analysis: `{ts, total, open, addressed, failed_categories}`. The arithmetic is `finding_statuses`' summary, the same one `metrics.record_findings` and the client's `findingCounts()` read, so the arc cannot drift from the board (N3: no second counter anywhere). `_record_pass(m)` hooks the append into BOTH analyze completions (full run and retry-failed) and is best-effort on purpose — a history write that fails must not answer a finished analysis with a 500 that pushes the writer into re-running the model; it prints instead.
**The endpoint stays honest about damage:** `GET /api/projects/<name>/passes` lets the store's own contract answer — a damaged arc is a 503 through the global `StoreUnreadable` handler, never `{"passes": []}`, because that reads as "you have never analysed this" to a writer with a history AND the next append would then rename over the only copy of the arc.
**One line, not a dashboard:** `buildPassArcLine` + `passArcSparkline` render "Pass 5 · 62 → 19 open · converging" under the arrival strip. The trend word is derived from the same two numbers the inline-SVG sparkline plots; hover is the native `title`, which names every point. One point renders "Pass 1 · 6 open" with no arrow and no trend word, because one point cannot move. `.dock-pass-arc` is tokens only — `--lamp` is the progress colour in every register, and "widening" keeps neutral ink because it is a fact about the draft, not a severity.
**Chronology is file order, not a sort:** an append can only happen under the store lock and it lands at the end, so the file IS the timeline; `ts` is when the pass was measured, and two passes inside one Windows clock tick (~15 ms) tie, which a sort would shuffle. Proven by a two-real-child-process leg.
**Gates:** `tests/test_pass_history.py` 6/6 (shape + arithmetic, order, missing → `[]`, damaged → `StoreUnreadable` on read AND write, concurrency) · `TestPassHistoryEndpoint` in `tests/test_webapp_api.py` 6/6, RED-proven load-bearing: commenting the two `_record_pass(m)` calls fails exactly the three arc legs, restoring them greens them — so they test the HOOK, not the route · a `StoreCase` in `tests/test_store_fault_injection.py`, which was not optional: `test_every_store_writer_is_in_the_registry` failed first · `tests/e2e_browser_pass_arc.py` 16/16 with two REAL demo analyses, so the "Pass 2" line is read from a store the analyze route actually filled · `node --check` OK · `ruff` clean · pytest 1661 passed / 3 skipped · battery 39 suites 38 passed / 0 failed / 1 skipped (`gun_pen_audit` wants a live llama-server) · `_p1_visual_gate.py` FAILURES: none, night + dawn.
**Known limits, stated:** the client's fetch swallows errors, so a damaged arc shows no line and no explanation in the browser — the server is the loud half (503 on read, traceback on write). The sparkline and its tooltip list every point, so a project with dozens of analyses gets a crowded line; the windowing upgrade path is noted above `passArcSparkline`. And `open` is the ledger's count AS OF each analysis: marks made after the newest run are not counted until it runs again — the line's own title says so, because a convergence line that borrowed unverified progress would be the UI pretending.
...
**What shipped:** `POST /api/projects/<name>/quickcheck -> {findings, errors, provisional: true}`, running `run_continuity_analysis(doc)` + `check_formatting(doc)` on `revision.load_working(m)`. No model client, no manifest write, no report, no progress heartbeat, and deliberately NOT under `_analyze_lock` — a test holds the lock and calls it anyway. It answers 400 when there is no script to check, so `ensure_working`'s `FileNotFoundError` cannot escape as a 500, and a rule pass that raises lands in `errors[]` instead of failing the request.
**The client keeps it out of the ledger's authority:** `state.lint` is its OWN layer, never merged into `state.findings`, so `findingCounts()`, the mass strip, the ink and the ONE filter cannot see it — a leg asserts `findingCounts()` stays 0/0/0 while rows are on screen. `buildLiveCheckSection()` mounts beside the report's own formatting rows AND in the *unanalysed* branch of `renderDockEvidence`, which is what lets the chip appear with no report at all. Its header carries `.dock-lint-chip` — "provisional — full pass pending"; rows print `[SEV] S1 — message` with a mono rule tag; a clean run says the rules saw nothing rather than vanishing, so the undo leg cannot pass vacuously; and a failed one says the last full pass is still the truth. `runQuickcheck()` is sequence-guarded — undo-then-redo fires two and the older answer must not land last — and re-renders the lens only: no re-ink, no re-count.
**Deleted:** the duplicated `await loadScriptData(); refreshAllFindingSurfaces();` tail at all six working-copy edit sites (inline save, apply-one, apply-many, undo, redo, reset), collapsed into one `afterScriptEdit()`.
**Gates:** RED confirmed at both levels first — 7 API legs (incl. no-llama-server, mutates-nothing, and reads-the-working-copy proven by a `long_action_block` that exists only after the edit), then `tests/e2e_browser_quickcheck.py` with 5 named failures before any client code existed. `node --check` OK; `ruff` clean; `pytest tests/ -q` → **1643 passed / 3 skipped**; `run_browser_suites.py` → **37 suites: 36 passed / 0 failed / 1 skipped** (`gun_pen_audit` still needs a live llama-server); `_p1_visual_gate.py` → FAILURES: none. Next: T19 (cards never cover the page; ink and click share one matcher).

- **2026-09-23 — ONE DESK, ONE LEDGER / P2.16: a partially failed report now offers both reruns, in one place, with the churn warning attached to the one that causes churn.**
*(plan Task 16, spec §15.2)*
**What was wrong:** `retryFailedCategories` was wired to three buttons — the desk toolbar, the drawer header, and (until P2.14) the arrival strip — each with its own label and its own disabled dance, and none of them ever mentioned the *other* rerun. So a writer who edited the script and wanted fresh eyes had to find the Analyze button two surfaces away, and a writer who just wanted the missing passes back had no idea the cheap option existed. Spec §15.2 asks for BOTH, side by side, honestly labelled.
**What shipped:** `buildFailureBanner` renders a `.failure-banner-actions` row — primary `.rerun-failed` ("Rerun the N failed passes", title: "Fast, and your good findings stay exactly as worded"), secondary `.rerun-full` ("Rerun the whole analysis", title: "…the model may re-word findings it already gave you, so your fixed/new counts get noisy unless you have edited the script"), plus a `.failure-banner-rule` line stating the rule of thumb in the writer's terms. The secondary calls the EXISTING `runAnalysis()` — same `force: true` POST, same stage ladder — so no second lifecycle was introduced; the primary keeps the merge endpoint.
**Deleted:** `#desk-retry-failed-btn` and `#retry-failed-btn` (both from `index.html`), their two listeners, the `retryBtn` sync in `refreshDeskToolbar` and `loadFeedbackPanels`, and the now-unreferenced `.retry-failed` chip CSS. `retryFailedCategories(anchor)` shrank with them: it used to hunt both global buttons and reset three labels in its `finally`; it now handles only its own anchor and re-renders the lens unconditionally, which is what makes a stale "1 pass failed" banner impossible.
**Wording moved, not dropped:** the desk's partial-report status line tells the writer where the action is ("The Evidence lens offers the rerun") and the dashboard's `⚠ N failed` tooltip now says the ledger reruns them.
**Gates:** RED first (7 legs, incl. the two POST contracts captured with `page.route` so the suite verifies what the client *sends* without paying for a real re-analysis). `node --check` OK; `ruff` clean; `pytest tests/ -q` → **1636 passed / 3 skipped**; `dock_sections` → **150 passed / 0 failed** (7 new P2.16 legs); `run_browser_suites.py` → **36 suites: 35 passed / 0 failed / 1 skipped** (`phase8_lifecycle` and `phase14_signoff_journey` were retargeted from "the desk chip mirrors the failed state" to "no rerun verb is clickable on the desk at all"); `_p1_visual_gate.py` → FAILURES: none. The gate cannot photograph the two-action banner — `gun_pen_2` is a clean report, where the banner is correctly absent — so the split's pixels rest on the seeded suite. Next: T17 (`/quickcheck`).

- **2026-09-23 — ONE DESK, ONE LEDGER / P2.15: the progress chip stopped inventing a percentage and started reporting the run's own heartbeat.**
*(plan Task 15, spec §15.1 — "no fake percentages — only real stage events; elapsed time, not invented progress")*
**What was wrong:** the chip filled a bar by *stage weight* and printed an extrapolated ETA. A pass that took 4× what its weight predicted still moved the bar exactly as planned, so the bar measured nothing — and `formatETA()` divided that fiction by elapsed time to promise a finish the server never said anything about. §15.1 had already ruled both out.
**What shipped:** `STAGE_LADDER` — **20 entries**, one per stage key `pipeline.py:analyze()` actually emits through `progress_cb`, each with a plain-language `caption` ("Reading dialogue — who sounds like whom") and a `title` for hover/focus. `renderStageLadder(container, currentKey, startedTs, timeoutSeconds)` marks rows done ✓ / current ● (pulsing) / pending ○; `stageElapsedText()` reads the event's own `ts`, so the current row says `working… 12s on this pass` and — past 2× `state.config.timeout` (the client's copy of the server's real 600 s default) — `no word from the model — waiting 21m 4s`. That second wording is the heartbeat's documented purpose: silence is reported as silence, not drawn as progress. A pass that reports a problem says so in the same line (`⚠ … reported a problem — the rest of the run continues`) instead of disappearing from the bar.
**Deleted:** `ANALYSIS_STAGES`, its per-stage weights, `ANALYSIS_TOTAL_WEIGHT`, `formatETA`, `renderPipelinePopover`, the `%` readout, the ETA, `.ap-bar` — plus the four now-dead `.ap-bar*` rules in `tungsten.css`. The poller keeps `p.ts` (`beatAt`) instead of trusting a client clock, and the rail re-renders only when the run *moves* (`ladderFor`); the ticking seconds patch one text node, so a hovering writer doesn't have 20 rows replaced every 800 ms.
**Layout constraint worth remembering:** the rail is a hover/`:focus-within` popover, not permanent desk chrome — `#desk-toolbar` is contractually ≤64px (`phase8_lifecycle`) and Task 19 forbids cards covering the page. What stays always-visible is the honest part: caption + elapsed. Keyboard path is the chip's existing `tabindex="0"`; `@media (prefers-reduced-motion: reduce)` kills the current-stage pulse.
**Gates:** RED confirmed first (`ReferenceError: renderStageLadder is not defined`). `node --check` OK; `ruff` clean; `pytest tests/ -q` → **1636 passed / 3 skipped**; `python tests/e2e_browser_phase8_lifecycle.py` → **25 passed / 0 failed** (16 new ladder/heartbeat legs, incl. *no `%` character anywhere in the rail*, unique stage keys, every row captioned, and a stale-`ts` case that must say "waiting"); `run_browser_suites.py` → **36 suites: 35 passed / 0 failed / 1 skipped**. Two suites still asserted the retired surfaces and were retargeted, not relaxed: `phase12_visual_motion` (`pipeline-pulse` → `stage-pulse` in the infinite allowlist, `.ap-bar-fill` dropped from the progress-meters list) and `phase14_signoff_journey` (its `progress: percentage renders` leg became "the live pass names itself / no percentage is printed / elapsed heartbeat renders / ladder mounted"). Next: T16 (retry split — one banner, failed-only default).

- **2026-09-23 — ONE DESK, ONE LEDGER / P2.14: the desk now says who wrote the report, what it could not finish, and what it got right — and the verification readout has one denominator.**
*(plan Task 14, spec §3 honesty + §6 arrival + §14.2 pin-to-notes)*
**What was wrong:** `report.md` has printed the analysing model, the per-pass errors and the coverage extras for months; the desk dropped all three. So a writer could not tell which model wrote a judgment, could not see that a pass failed *without findings* (a silent hole read as a clean bill), and met the ledger as an all-problems list with nothing about what already works.
**What shipped (5 surfaces, all `escapeHtml`/`textContent`, tokens only):** `.dock-report-model` (`buildReportModelLine()` basenames `model_used` on **both** separators — a real report's value is a full Windows GGUF path, and printing `C:\Users\…` on a desk UI is a privacy leak disguised as provenance; it baselines with `String.fromCharCode(92)` rather than an escaped backslash because a bash heredoc eats `\\`); `.failure-banner` (`buildFailureBanner(failed_categories, report.errors)` — head is "N pass(es) failed — rerun just that" when passes failed and "N pass(es) reported a problem — nothing was skipped" when only errors arrived, one `.failure-banner-error` per string, mono); `.dock-working` (`buildWhatsWorking()` from `coverage.strengths`, "What's working (N)", above the problems per §6); `.dock-section[data-key=formatting]` (deterministic `formatting_findings` labelled "not model judgment", collapsed, rows not cards); and the mirror's `.wm-confidence` + `.wm-scenes`, rendered only when the key exists because real `character_reads` carry no `confidence`. Coverage gained `.dock-cov-frame` (genre · tone) and `.dock-cov-films`.
**The two-denominator bug from T11 is fixed here:** `verificationReadout()` is the ONE builder behind both `.dock-trust` renderings (arrival strip + script-mass strip) — `checkable = verified + not_found + scene_not_found`, with `no_quote` stated as its own clause and never inside the denominator. The visual gate now fails if the two strings disagree.
**Pin-to-notes (§14.2):** `.finding-pin-btn` lives in the card's **action row** (T13's lesson — `.finding-deep` is hover-only, so a verb inside it can never be clicked). `pinFindingToNotes(f)` POSTs `{scene_number, text: issue + " — “quote”", anchor}` to the EXISTING notes endpoint (no new backend), then `reloadNotesAndRender()` and `paintPinState()` — so pin state is derived from `state.notes`, survives re-render, and cannot double-pin. There is no toast helper in this SPA and the existing note-POST convention has no success message, so the pinned label *is* the confirmation.
**Deviation worth knowing:** the arrival strip's inline retry block was **deleted**, not duplicated — the banner is now the only failed-pass retry, and `retryFailedCategories(anchor)` re-renders the lens in its `finally` so a "1 pass failed" cannot outlive the failure it describes. T16 therefore only adds `.rerun-full` to the banner and removes the desk-toolbar + drawer buttons.
**Gates:** `node --check` OK; `ruff` clean; `pytest tests/ -q` → **1636 passed / 3 skipped**; `dock_sections` → **143 passed / 0 failed** (30 new P2.14 legs, incl. a real `POST /notes` captured via `page.on("request")` and counted back from the server, and an `<img onerror>` payload inside a strength string that must reach zero img nodes); `_p1_visual_gate.py` → FAILURES: none, with 4 new photographs (`03g_report_model_night`, `03h_whats_working_night`, `03i_formatting_row_night`, `07d_whats_working_dawn`) and pixels rules for the model line (no machine path, not clipped), the strengths head, the formatting title, banner-absence on a clean report and readout agreement. `tests/e2e_browser_gun_pen_audit.py`'s two assertions off the retired `.dock-arrival-retry` class were retargeted to `.failure-banner` count 0. `gun_pen_audit` itself still needs a live llama-server, so the banner's *presence* is proven only by the seeded suite. Next: T15 (stage ladder).

- **2026-09-23 — ONE DESK, ONE LEDGER / P2.13: a card that cites a craft rule can now say which one, and who says so.**
*(plan Task 13, spec §7 — honesty fields the markdown report already had and the desk dropped)*
...
- **2026-09-23 — ONE DESK, ONE LEDGER / P2.12: a queue row now says how well its own evidence held up, and one builder decides what that looks like.**
*(plan Task 12, spec §7 — "a to-do over a real finding" can't be dumber than the card it points at)*
**What was wrong:** `get_fixqueue` built its items through a field allowlist that dropped `evidence_quote` and `verification` on the floor even though `_sanitize_report` keeps both — so the deep card could say "⚠ unverified" while the row that leads to it said nothing, and the two badges were separately written inline code that could drift. **The three edits:** the allowlist hands both fields through (the client renders them verbatim through `escapeHtml`, as everywhere); a new module-level `verificationBadge(v)` is the ONLY place a verification status becomes pixels, and both `renderFixQueuePanel`'s row and `findingNoteEl`'s card call it; the non-verified wording stays the card's existing `⚠ unverified` with the precise server status (`not_found` / `no_quote` / `scene_not_found`) in the badge `title`, so no new vocabulary entered the ledger.
**The regression the pixels caught:** the row badge took a line away from `.fix-row-body` — in the ~380px ledger column the issue text fell to one word per line and a row stood **486px tall**. Fixed in CSS only (`.dock-section .fix-row { flex-wrap: wrap }` + body `flex: 1 1 100%`, the shape `.revision-findings` already uses); re-measured 257px with 36 badges rendered. `_p1_visual_gate.py` now fails on any queue row over 260px, so row-height damage can't come back silently.
**Gates:** `pytest tests/test_fixqueue.py tests/test_feature_batch.py -q` → 22 passed (new `test_items_carry_evidence_and_verification` asserts the quote that arrives is one the report actually contains, and that verification crossed at all); full `pytest tests/ -q` → **1633 passed / 3 skipped**; `dock_sections` → **98 passed / 0 failed** (3 new legs compare each row's DOM badge against `verificationBadge(item.verification)`); `counting_contract` → **20/0**; `_p1_visual_gate.py` → FAILURES: none. `gun_pen_audit` still needs a live llama-server. Next: T13 (card `verification.note` + `check_id`, and a `GET /api/rules/<id>` popover endpoint that does not exist yet).

- **2026-09-23 — ONE DESK, ONE LEDGER / P1 gate: the pixels have now actually looked at the ledger (night + dawn), and one thing they rejected is fixed.**
*(plan Task 11 — the step whose whole point is that green DOM assertions are not visual truth)*
**New harness:** `tests/_p1_visual_gate.py` — self-booting like the other `_`-prefixed walks, but it serves a **copy** of the analyzed `gun_pen_2` project (255K, real report + `last_pass.json`) into a temp dir instead of the demo sample, because this gate is about real findings depth. Each shot is taken with the layout facts read in the same instant (`MEASURE` → `impl-shots/runs/latest/p1_visual_gate.json`), so a claim and its image cannot drift apart.
**What the pixels accepted:** the ledger opens with **0 of 9 sections expanded** on a fresh load; the arrival headline reads `3 of 36 addressed by you` + `Start the fix loop — 14 highs`; the manuscript holds **70.4%** of the viewport with the dock open (96.8% closed) — spec §11's ≥50% holds with room; no surface overlaps, `scrollWidth - clientWidth == 0`, no JS errors; the P1.10 shelf shows ONE Pacing panel with both named metrics and the `Fix queue: open the ledger →` defer; the deep card's `✓ you fixed it — survives re-analysis · ⏭ park it — returns next analysis` hint renders in dawn from tokens; and the This-scene chip was measured **both directions** on real data: 36 queue rows → 18 → 36.
**What they rejected (fixed):** in the ~380px ledger column the fix-queue head put its count line and the dawn meter side by side, so `FIX QUEUE — 33 OPEN / 36 SHOWN / 36 TOTAL` tangled into three lines with `NIGHT ▬ 8%` jammed mid-sentence. `style.css` now stacks them inside `.dock-section .fix-queue > .craft-panel-head` (column, `align-items:flex-start`, meter loses `margin-left:auto`) — count line on one row, meter under it, in both registers. CSS only; the title string is untouched, so `counting_contract`'s pin on it still holds.
**Two harness lessons worth keeping:** (1) the desk's chrome is `opacity:0` 4s after the pointer leaves the top 120px (`auto-hide-chrome`), so a naive screenshot shows an empty band where the toolbar is — the gate parks the mouse at `y=40` before each shot; (2) `#context-dock.open` in the DOM is NOT proof the writer can see the dock (Escape closes it, class can survive) — `ledger()` now asks whether the tab itself is visible.
**Filed, not fixed here (it is §7/T14 work):** the same verification fact is printed with two denominators side by side — the arrival strip says `10 of 36 quotes verified (28%)` while the ledger's mass strip says `10 of 10 quotes verified (100%) · 26 carried no quote`. Both are true; read together they look like a contradiction. T14 should pick one denominator per surface and name it.
**Gates:** `python tests/_p1_visual_gate.py` → FAILURES: none; pytest **1632 passed / 3 skipped**; `counting_contract` **20/0**; `dock_sections` **95/0**. `gun_pen_audit` still cannot run here (no llama-server on :8080), so the P1.8 dedupe remains unverified against a *live model* pass — this gate covers the *stored* Gun_Pen report only. Next: T12 (`/fixqueue` carries evidence + verification; queue rows badge verification state).
...
- **2026-09-23 — ONE DESK, ONE LEDGER / P1.8: the ledger now cards each finding ONCE, and the mass strip stopped counting.** *(plan `docs/superpowers/plans/2026-09-22-one-feedback-room.md` Task 8, spec §5 "one rendering per finding per panel" + the §8 counting rider the plan never tasked)* **What was wrong, measured:** the Evidence lens has three card sections and the categorized "Live findings" list is a view over EVERY filtered finding, so the same finding was carded twice — on the demo report the DOM answered `6 (scene-findings + by-category)` and `7 (script-level + by-category)`. **The fix is one function and one set:** `dedupeDockFindings(...)` returns the indices the higher-precedence sections already rendered, and `renderDockEvidence` feeds it `sceneFindings` then `scriptLevel`, so the categorized list is the **catch-all** (precedence this-scene strip → script-level → live list — the most contextual rendering wins) and the `Live findings` header tooltip now says how many were carded above it, so its count cannot be misread as the whole ledger's. **Deviation from the plan's stated precedence, with the reason:** the queue is deliberately NOT a claimer — `webapp_server.get_fixqueue` emits exactly one row per report finding, so letting `queueIds` into the skip-set would empty the categorized ledger on every analyzed project, and §5 wants both sections (the queue row is the compact to-do with Locate/Rewrite/Discuss/Dismiss; the card is the evidence view with quote + verification + intent). Recorded in the plan under Task 8, not silently. **The §8 rider:** `buildScriptMassStrip` still walked `state.findings` itself for its headline, its severity mass and its category weights — the seventh counting path, in the one place the writer reads as "the truth about the whole script". `findingCounts()` now weighs the open findings on its own pass (`bySeverity`/`byCategory`) and the strip is pure presentation; `test_app_symbol_integrity.py::test_mass_strip_reads_the_one_counter` fails the build if the strip ever calls `findingOpen(` or iterates again. **Tests (`tests/e2e_browser_dock_sections.py`, 67 → 73 checks):** `LEDGER_DUPES_JS` asks the DOM — `data-finding-index`, stamped by `findingNoteEl` — which section cards each finding, asserted on the real report AND the seeded fixture in both chip states; `live_cards()` was re-pinned from "cards in one section" to "cards in the ledger", which made P1.7's ink/list parity checks **stronger** — they had been passing while cards were duplicated precisely because they only looked inside one section. **Two browser suites were already red on `qoder/update` before this commit, and it is not this work:** `phase8_lifecycle` (`Locator.click` timeout on `#desk-analyze-btn` — `#manuscript-workspace` intercepts pointer events) and `translate_mic` (the idea page's aria snapshot). Proven pre-existing by running both in a clean detached worktree at `0d12d56` — both exit 1 there too; they need their own investigation, and P1.6/P1.7 (the commits that likely moved them) carry no NOTES entry. **Gates:** `node --check` OK after every app.js edit; pytest **1630 passed / 3 skipped**; browser runner 36 suites → 33 pass, 1 skip (`gun_pen_audit` needs a real llama-server), 2 pre-existing fails above; `dock_sections` 73/73, `counting_contract` 20/20; `ruff` clean. Next: T9 (arrival strip inversion + fix-loop CTA), T10 (shelf defers, the one Pacing, vocabulary unification).

- **2026-09-22 — ONE DESK, ONE LEDGER / P0 SHIPPED: the subtraction phase — four historical feedback surfaces became one ledger's worth of counting.** *(spec `docs/superpowers/specs/2026-09-22-one-feedback-room-design.md`, plan `docs/superpowers/plans/2026-09-22-one-feedback-room.md`; five commits `77969fb`, `05d4e02`, `440f5bb`, `baaaad0`, `93d5f0f`)* **P0.1** deleted the dormant `#feedback-view` clone — markup + the `renderFvBoard` family + call sites (~500 lines; the clone's `.fv-*` CSS was **deliberately kept**: the `fv-` prefix is shared with the LIVE dock consult/cowrite chat — `renderFvChat`/`fv-consult`/`sendFvMessage` — the exact shared-prefix trap Task 1's review caught before a blanket delete repeated the 2026-09-10 careless-delete lesson). **P0.2** retired the Problem Board entirely (auto-open, `renderProblemBoard`/`pbItemClick`/scroll-sync/`#pb-filter`/`#pb-edge-tab`, ~443 lines incl. `.pb-*` CSS + the `--z-board` token; verified per-rule — `--z-board-tab` and `--elev-tab` kept, they have live consumers), relabeled the palette entry that lied about key `b` ("Toggle the Problem Board" actually opened the Beat Board), and added `_sanitizeLegacyState` (drops `pbOpen`/`problem_board`, maps stored `view:"feedback"`→`"desk"`; the actually-retired value `"fv"` was already migrated by the restore branch). **P0.3** made ONE counting path real: `queueCounts()` + `queueItemFindingIndex()` map `/fixqueue` items through `findingDisposition`, and the fix-queue header, the **craft-shelf lid** (a sixth surface the plan missed — it sat directly over the queue contradicting it), `#finding-summary` chips, dawn meter, revision strip and arrival draft clause all print from `findingDisposition`/`findingCounts` now; the divergent `findingStatusSummary` was **deleted**, not reconciled; the dawn meter gained a return-value stamping fix (a pre-existing invisible bug — its `querySelectorAll` could not reach the shelf's unmounted elements). **P0.4** added `refreshAllFindingSurfaces()` — the ONE re-render entry point every mutation calls (dismiss, intent, edits apply/undo/redo/reset, inline edits), with `refreshMetrics()` riding along so the status strip stops waiting for the next `openProject`; queue rows now derive their `done` state from disposition too. **P0.5** re-pinned the suites that asserted the retired surfaces: phase5/phase6 legacy loops inverted (live two stay, retired two assert ABSENCE), phase13's board section (visibility/geometry/edge-tab/gutter walk/spotlight ~90 lines) became a retirement gate + the surviving Context-affordance check, phase7's Esc check now asserts the dock closes rather than "returns to the Feedback View", and **`xss_inert` was re-pointed, not weakened** — payload legs now drive the Evidence dock canvas, dead-sink legs assert `typeof renderFeedbackView === 'undefined'` (absence is the strongest form of inert), all 21 security checks green including the CSP enforcement probe. **Gates:** `node --check` after every app.js edit; pytest **1629 passed / 3 skipped**; browser runner **35 suites — all green** after re-pins (counting_contract 19 checks, smoke 18, phase13 27, phase5 23, phase6 33, phase7 14, xss 21); `tests/test_app_symbol_integrity.py` gained 2 grep-gates (clone gone, board gone). **Lesson recorded:** the token-wise "delete the whole prefix" instinct was wrong twice in one task — check EVERY call site before deleting a symbol or a CSS rule that shares a prefix with live code. Next: P1 (ledger rebuild: real collapse, live-highs default + this-scene chip, one-rendering-per-finding, arrival inversion + fix-loop CTA, Pacing merge).



- **2026-09-21 — The audit was overwriting the evidence its own verdict tables cite; the guard for it was wrong twice before it was right; and `git add -A` swept a concurrent session's files into a commit.** *(pass 14h — harness only, no product code)* `FULL_FEEDBACK_AUDIT_VERDICTS.md` filed this itself as an aside — *"A verdict table whose screenshots get silently replaced by the fix is a small honesty bug of the same family this document exists to catch"* — and it was disclosed, never fixed. Re-running `e2e_browser_gun_pen_audit.py` wrote its shots straight into the top-level `impl-shots/`, the **versioned evidence set** the verdict tables and `docs/audit/` cite by exact filename, so the ordinary act of re-running the audit replaced a table's screenshots with post-fix images — and `impl-shots/audit_results.json` (also cited, also tracked) with them. The repo already had the right pattern and the audit did not follow it: `tests/_ui_capture.py` writes to gitignored `impl-shots/ui_audit/` ("regenerate on demand") and `tests/test_repo_hygiene.py` exists to keep local scratch out of the index; the audit was the one harness writing into a tracked directory. **The fix is the rule, not the instance:** a run writes to gitignored `impl-shots/runs/latest/` — shots *and* results together, so the two can never disagree about which pass they describe — and reaching the versioned set is the explicit act `AUDIT_PROMOTE=1`, which prints that it is replacing cited evidence. The scratch path is deliberately **stable** rather than timestamped because stages are separate processes that accumulate through `audit_results.json`, so a per-run directory would make a later stage silently start from nothing. **The guard was wrong twice.** (a) The mutation harness could not apply its own mutations — the audit file is CRLF and `.gitignore` is LF, so anchors written with the wrong endings silently failed to match and two of three mutations reported `ANCHOR NOT FOUND`, which the harness printed as a skip rather than a pass and would have read as a clean 1/1. (b) **The guard used the wrong git flag and was vacuous:** `_is_ignored` called plain `git check-ignore`, which reports a **tracked** file as *not* ignored — the index wins — so the test could not fail until after the damage and was blind to the `.gitignore` rule that causes it; mutation M8 proved it (the tempting wrong fix, gitignore the whole `impl-shots/`, **SURVIVED**), and `--no-index` — ask the *rules*, not the index — catches it. **Verified:** `test_repo_hygiene.py` 6 passed (3 new) covering the rule's three halves (default is ignored; promotion still reaches the versioned set; the evidence is never untracked), **mutation-verified 3/3** as named failures with the source restored byte-identical; `ruff` clean. **Separately found:** commit `4fb3a73` contains 309 lines it should not — `docs/superpowers/specs/_expert-ux-critique.md` (248) and `_expert-dataflow-audit.md` (61), written by another session working in this repo minutes earlier. `git add -A` had a wider blast radius than the intent, the identical defect class to the `rmtree` sites and the AST walk's glob, this time in the staging step; those files are left in place (deleting another session's work would be worse than the misattribution) and commits from here stage explicitly, with `_expert-redteam-optionA.md` left untracked and untouched.

- **2026-09-21 — "Flag, don't drop", swept repo-wide: 21 broad-and-silent handlers, 4 that mislead.** *(pass 14g — production)* The project states the rule and lint enforces only its crudest form (`E722`, bare `except:`). A sweep for handlers whose body is **only** `pass`/`continue`/bare `return` found **21 broad-and-silent** handlers across the four packages. Most are defensible — optional-module imports, telemetry, skipping an unreadable project during a shelf scan — and carry comments saying so. **Four were not, and in all four the *consumer* is misled.** (1) **`_build_doctor_case_file` misleads the MODEL:** the file is handed over as the writer's *"whole shelf"*, and a script whose manifest or report could not be READ was dropped without a word — so every number below it was wrong while looking authoritative and the doctor reasoned from an incomplete history without knowing. It now discloses `NOTE: N script(s) on the shelf could not be read (…) and are NOT counted above`. (2) **`SessionStore.list()` misleads the WRITER:** a damaged session file made a conversation vanish, and *"never had one"* is indistinguishable from *"it's gone"*. (3–4) **`graduate_idea` ×2:** a failed carry and a failed session pin both left the conversation silently absent after graduation. The distinction the sweep encodes: **a script that was never ANALYZED is legitimately absent; one that could not be READ is damage** — the rule the writer's stores already follow (A3: *"missing is a legitimate empty; damage is reported"*). The other **16 are reported, not changed** — each is a deliberate best-effort path with a comment saying why, and turning them into failures would break the shelf scan or the request they exist to protect; recorded so the count is a known number rather than an unknown one. **Verified:** `test_humanization_v2.py` +1 pinning the disclosure (including that it NAMES the script), mutation-verified as a named failure with the source restored byte-identical; **267 tests** across ten affected suites pass (3 expected structural skips). **And the gate went red for a fifth unrelated reason:** `phase14_signoff_journey` ERRORed with `Connection.init: Connection closed while reading from the driver` after **2 seconds and zero checks** — the Playwright driver died on startup — and passed **47/47 when run alone**. `run_browser_suites.run_one` now retries **once**, and only when the failure matches a driver-init marker *and* produced no check summary (so a genuine failure is never retried away), and **reports the retry in the detail** so a retried pass can never be mistaken for a first-time pass; both halves are pinned in `test_production_readiness.py`. Gate back to **34 suites — 33 passed, 0 failed, 1 skipped**. **That is the fifth instrument to lie in this session** — a sandbox proxy, a Chromium-blocked port, my own AST walk, that walk's glob, and now the browser driver. None was the product.

- **2026-09-21 — My sweep only walked one package, and the app had no JSON backstop.** *(pass 14f — production)* The pass-14e sweep globbed `screenplay_studio/` — the module I was already looking at — so it could not have found the same defect class elsewhere. Widened to all four packages, it turned up the one path **AGENTS.md names explicitly**: `screenplay_parser/pdf_parser.py`'s `_ocr_extract` removes its per-page temp PNG in a **`finally`**, unguarded. A raise from a `finally` **replaces the in-flight exception**, so a refused temp file would turn the clear *"a clear message, not an empty parse"* OCR error into a traceback about a PNG — the exact same shape as the `upload_draft` bug one pass earlier, in a different package. Four more unguarded single-file removals (`cowriter/store.py`, `beatboard.py`, `diff.py`, `revision.py`) are reachable from routes whose own `except` clauses catch only `ValueError`/`FileNotFoundError`, so an `OSError` escaped them. **And the sweep had a second false-positive mode:** a call inside a `finally` was reported unguarded even when the finally body guarded it itself (which `stt.py` does) — the `elif` chain let the outer `finally` short-circuit the inner guard. **The systemic fix beats six call-site guards:** the app now has ONE `@app.errorhandler(Exception)` that returns JSON — passing `HTTPException` through untouched so 404/405/413 keep their codes — because every route's failure path already returns `_error(...)` (the SPA reads `error` off the body) while an unguarded raise anywhere else produced Flask's **HTML 500** and the writer was told nothing. The per-site guards stay (they name *what* failed); the handler is the floor beneath them. **Tests:** `test_destructive_paths.py` 5 (2 for the backstop), `test_pdf_ocr.py` +1 pinning the OCR information-loss property — **five mutations, all named failures, source restored byte-identical**.

- **2026-09-21 — The last two unguarded destructive calls, and a bug in the sweep that found them.** *(pass 14e — production)* Pass 12 swept every destructive filesystem call with an AST and split them by **blast radius**: the two multi-file `rmtree` sites could leave a PARTIAL result, so they were fixed; the single-file `os.remove` sites *"either happen or raise"*, so they were recorded and left alone. **That split was right about state and wrong about reporting.** A raise is not free — the front-end reads `error` off the response body, so an unguarded raise answers with Flask's **HTML 500** and the writer is told nothing they can act on. Both remaining sites are now guarded, and one was worse than recorded: `reparse_project`'s artifact-drop loop escaped the handler; `upload_draft` removed its temp upload in a **`finally`**, and **an exception in a `finally` REPLACES the in-flight exception** — so a failed cleanup discarded the clear `"Could not process new draft: …"` error returned just above and escaped as a generic 500. The writer lost the real reason. The two treatments differ on purpose: **reparse fails loudly** (`"Re-parsed, but could not drop the stale analysis file report.md: …"`) because silently continuing would leave a stale report on screen against a fresh parse — the "stale state shown as current" lie this desk is built not to tell; **draft cleanup is non-fatal** (it is not the operation, the file is a temp upload the writer never sees, and the next upload overwrites it) and reports the refusal. **And the sweep that produced the pass-12 table was itself buggy:** re-running it flagged five route-handler sites as unguarded, and reading them showed **three were already inside `try` blocks** — the guard test asked whether a line fell inside the **first statement** of a `try` body rather than inside the body's span, and the first statement is one line, so only calls on that exact line looked covered. Fixed, the sweep agrees with the pass-12 table exactly: **two** unguarded sites. **That is the third instrument to lie in this session** (a proxy, a blocked port, and my own AST walk); each correction came from reading the code the instrument pointed at. New `tests/test_destructive_paths.py` (3 tests), **mutation-verified 2/2** with the source restored byte-identical; the `finally` test pins the information-loss property, not just the status code — it asserts the response still carries `Could not connect to llama-server`.

- **2026-09-21 — The gate was flaky by construction, and the unit suite cannot complete in this session.** *(pass 14d — harness only)* Re-running the 34-suite browser gate after the shared-harness change came back `34 suites: 32 passed, 1 failed, 1 skipped`, and the failure had nothing to do with the change: `ERROR smoke — Page.goto: net::ERR_UNSAFE_PORT at http://127.0.0.1:2049/`. `free_port()` drew a random free ephemeral port, and **Chromium refuses to navigate to a list of ports it considers unsafe for the web** (2049 is the NFS port) — so the port was free and unusable, and the suite died with a message that reads like a broken product. `free_port()` now redraws until the port is free **and** off Chromium's blocked list (200 draws verified safe); the gate is back to **34 suites — 33 pass, 0 fail, 1 skip, 0 known-broken** with `smoke` passing 18 checks. **The second item is a limit, not a defect.** A full `pytest` run **cannot complete in this session, reproducibly**: the sandbox hook's counter (`scope:"turn"`, `threshold:50`) reached `count:14755`, so it fires on every full run and aborts tests with **`SystemExit: 1`** — the same two every time (`test_packaging_data_files::test_sdist_ships_the_data_files` errors on setup, `test_writer_memory::test_entity_scope_map_resolves_relative_projects_dir` fails), and both **pass or skip when re-run in isolation** (the first skips for a missing `setuptools`). Disabling the OS sandbox made it **worse** (1 error → 5), because the hook is not the OS sandbox. So: a full-suite red here is unread until each failure is re-run in isolation, results should be captured with `--junitxml` (a killed session loses the summary section entirely), and the trustworthy verdicts this pass are the browser gate and `gun_pen_audit`.
