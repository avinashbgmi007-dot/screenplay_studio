...
def run():
    ...
    with start_studio() as studio:
        with sync_playwright() as p:
            ...
            # the title flip is necessary but not sufficient — openProject
            # reveals .workspace in the same tick, so waiting on BOTH keeps
            # this check from racing the async open (the old title-only
            # wait could pass on a stale/mojibake placeholder)
