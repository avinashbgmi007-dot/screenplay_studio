...
**Still open after this gate (not blockers, all named):** ~40 orphan `.fv-*` selectors left in the sheet → **closed**, 43 dead ones removed and the family pinned by name in the entry above; commits on `qoder/update` remain unpushed (credential-path blocker); spec §2.2's doctor-gating orphan needs a product decision.
