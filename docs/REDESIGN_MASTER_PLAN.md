...
| Never-defined `var()` tokens | 6 (`--rust-flag`×3 js, `--shadow-paper`, `--ink-600`, `--accent-dim`, `--text-dim` masked) | **R0.6 DONE:** all resolved (error bubbles → `--danger`; `--shadow-paper` tokenized from the approved prototype; stash-float → real ink ramp; wd-keep → color-mix; snap → bare literal). Dead spring island (`.rise`/`.rise-pop`/`--spring`, zero refs) removed. Sweep now: 0 used-but-undefined. Remaining known debt: ~23 dead fallbacks on defined tokens (cosmetic, can never fire). |
