...
**Box 2 is NOT met, on purpose.** `app.js` is +944/-392 against `main`, not net-negative: every deletion bought a surface the same spec asks for (persisted collapsible sections, the one-rendering dedupe, the convergence line, the stage ladder, `/quickcheck`, the rule popover). The retired-surfaces half is met and pinned in three files. Amending the box is a product call, so it stays unchecked in §11 with the numbers next to it.
...
**Still open after this gate (not blockers, all named):** ~40 orphan `.fv-*` selectors left in the sheet (the classes themselves ARE live in the dock's chat slots — only ancestor-scoped ones are dead); commits on `qoder/update` remain unpushed (credential-path blocker); spec §2.2's doctor-gating orphan needs a product decision.
