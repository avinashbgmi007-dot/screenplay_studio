...
**HEAD:** `4fb3a73` (passes 14 → 14g) — **pushed**; `git ls-remote origin main` agrees. The
...
|---|---|---|
| Unit + integration | `python -m pytest tests/` | **1838 passed, 3 skipped, 0 failed** — measured 2026-09-26 at **`b1ef7d3`** (222s; coverage **87%** — 9775 statements, 1262 missed, against the `fail_under = 85` floor; the 3 skips are `test_store_fault_injection`'s structurally-inapplicable non-load-modify-write cases; **+11** over the pre-pass row: `test_spa_contract.py` 11 — the FE-M2 view union, the FE-M3 hidden-control contract and copy rule, and the FE-L1 scene-rail naming). The 1827 figure was measured at `dbeade7`; it was first recorded against `d19fbce`, which was wrong — the +2 BE-5 tests are in `dbeade7` |
| Lint | `ruff check .` | **All checks passed** (re-measured 2026-09-26 at `b1ef7d3`) |
| JS unit | `node --test tests/js/*.test.js` | **16 / 16** (re-measured 2026-09-26 at `b1ef7d3`) |
| Browser E2E | `python tests/run_browser_suites.py` | **55 suites: 54 pass, 0 fail, 1 skip, 0 known-broken** — **1,343 checks** — measured 2026-09-26 at `b1ef7d3`, **UNCHANGED across three consecutive revisions**. That is the informative part: those revisions edit `app.js` (loaded by all 55 suites) and `index.html` (adding a `role` and two `aria-label`s), so an unchanged fleet is the evidence that a comment rewrite, two copy strings and three attributes moved nothing else. `gun_pen_audit` remains the one skip — it POSTs a real `/analyze` and needs a live llama-server. |
...
| FE-M2 | The `state.view` declaration comment had drifted five names behind the code (`// "chat" \| "script"` against nine accepted names) | `613627e` | The comment now carries one machine-readable `// VIEWS:` line, and `tests/test_spa_contract.py` asserts it equals `openViewByName()`'s own literals plus the else-branch default. Mutation-verified: dropping `compare` from the line goes red. **Prose is not a data format** — the first extractor scraped `\|` tokens out of the whole comment and lost two names to a `//` prefix and an em-dash sentence |
| FE-L1 | The scene rail was an **unlabelled** navigation region: `#scene-index` had no `role` and no name, and the toggle's only accessible name was a `title` tooltip. *(The audit's item-level claim — "no labels, no accessible name" — is refuted: 3/3 items are named with their full scene heading)* | `b1ef7d3` | `role="navigation" aria-label="Scene index"` on the rail + an explicit toggle label. Measured at runtime: `role='navigation' aria-label='Scene index'`, toggle `aria-label='Toggle scene index'`, items still 3/3 named. `tests/test_spa_contract.py` (3 tests). Mutations M7–M10 |
| FE-M3b | *(missed by the 2026-09-21 audit)* **The product told the writer to look at the control it deliberately never renders** — `app.js` said `"Applied to the working copy — Undo is in the script toolbar."` while `#undo-btn` lives in `#desk-toolbar` and is `display:none` by design. A second site (`app.js:370`) used the hidden button's own glyph (`↶`) | `613627e` | Both now name the keyboard — the actual contract (`app.js:8627` binds Ctrl/⌘ Z to `undoEdit()`). `tests/test_spa_contract.py` asserts no copy names a *location* **and** that the post-apply hint still names the keyboard, so the rule cannot be satisfied by deleting the message. Mutation-verified: **10/10** across all three claims (M1–M10), three files byte-identical |
...
| FE-M2 | MED | `state.view` union has drifted, and a dead value is still consulted | **FIXED — half the audit's claim was wrong.** The stale comment was real: `app.js:18` read `// "chat" \| "script"` — two legacy aliases and **not one of the six live names**. It now declares the union on a machine-readable `// VIEWS:` line, asserted against `openViewByName()`'s own literals by `tests/test_spa_contract.py`. The second half is **refuted**: `"fv"` is not a dead value still consulted. `app.js:2143` documents `openViewByName` as "the ONE place a view name becomes a view. The saved session and the URL both go through it", and `app.js:2140-2142` says the legacy aliases "keep their ORIGINAL destinations rather than being collapsed, so restoring an old session behaves exactly as it did." `"fv"` is reachable from the URL parser (`app.js:2124`) and from a saved payload (`app.js:2047`), and `openFeedbackView()` exists at `app.js:7562`. It is a live compatibility shim |
| FE-M3 | MED | Undo/Redo are unreachable by mouse | **NOT A DEFECT — deliberate and documented, 11 days before the audit.** The audit's observation is true (`index.html:209-210` carry `style="display:none;"` and no JS ever assigns their `.style.display`), but it is the intended design: `index.html:206-208` says *"Undo/Redo: no visible surface on purpose — keyboard parity (Ctrl+Z / Ctrl+Shift+Z) is the contract; the hidden buttons stay because the edit-state refreshers drive their .disabled."* `git log -S` dates that comment to **`f648506` (2026-09-10)**; the audit is 2026-09-21. Deliberateness is also visible in the code itself: `app.js:5342` reveals `#reset-edits-btn` (`hasEdits ? "inline-block" : "none"`) while `5343-5344` only *disable* undo/redo — the same author, three adjacent lines, choosing differently. The audit's `✅ code-verified` label verified the code and not the comment above it. **Confirmed at runtime, not just in source** — with a real edit applied, in one browser: `#reset-edits-btn` computes `display: inline-block` (revealed) while `#undo-btn` and `#redo-btn` compute `display: none` (`.workbuddy-ai/scratch/contrast_runtime_probe.py`). Same toolbar, same inline starting state, three adjacent lines, different treatment. **The real defect was the copy, and the audit missed it** — see the Closed row below |
| FE-M3b | MED | *(the audit missed this)* The product told the writer to look at the hidden control | **FIXED.** `app.js` said `"Applied to the working copy — Undo is in the script toolbar."` while the toolbar's Undo button is deliberately `display:none`, so the writer was sent to a place with no control in it. A second site (`app.js:370`) referenced the hidden button's own glyph (`↶`). Both now name the keyboard — `Ctrl/⌘ Z` — which is the actual contract: `app.js:8627` binds it to `undoEdit()`. Guarded by `tests/test_spa_contract.py`, mutation-verified 6/6 |
...
| FE-L1 | LOW | Left scene rail is cryptic (`1 ▮ 2 3 c`), no labels, no accessible name | **REFUTED at item level, REAL one level up — and fixed.** Measured in a browser, every item already carries `role="button"`, `tabindex="0"`, a `title` and an `aria-label` of the **full scene heading** (`app.js:5412-5427`) — **3/3 named, 3/3 with a visible heading**, so "no labels, no accessible name" is false as written. `1 ▮ 2 3 c` is the *collapsed* 44px strip, which is the rail's design, not its content. The genuine gap was the GROUP: `#scene-index` had no `role` and no name, so a screen reader reached an unlabelled cluster of buttons, and the toggle's only name was a `title` tooltip. Both fixed (`role="navigation" aria-label="Scene index"`; explicit toggle label), verified at runtime, guarded by `tests/test_spa_contract.py` (3 tests) and mutation-verified (M7–M10) |
| FE-L2 | LOW | At 900 px the premise-doctor drawer takes ~48% and the canvas runs under it | **REFUTED — the container it describes was retired.** Measured at a 900×800 viewport with the premise view open: `#premise-view` is **900px = 100% of the viewport**, full-screen, and `#manuscript-container` is 0px wide with **0px overlap**. There is no ~48% drawer, and the canvas does not run under anything. The cause is in the stylesheet, self-documented at `style.css:838`: *"Phase 13: the `.premise-pane` container retired — the premise card is the full-screen `#premise-view` now."* The audit's `⚠️ reported` tier was right to withhold confidence — it described a layout the code had already replaced |
| FE-L3 | LOW | Four abandoned design labs still ship (~26 HTML files) | **COUNT CONFIRMED, FRAMING WRONG — and this row corrects my own earlier reading of it.** The 26 is exact: `preview-next` 7 + `preview-redesigns` 7 + `preview-r4` 8 + `preview-design` 4. But *"abandoned"* applies to three of the four. **`preview-next/` is live**: 2 of its 7 pages call `/api/preview/*`, which has five routes in `webapp_server.py` (`/api/preview/projects`, `/data/<name>`, and `GET`/`POST`/`DELETE /chat/<name>`) and 14 tests in `test_preview_lab.py`. The other three reference the API **zero** times and are inert mockups. All four ship deliberately — `pyproject.toml:53-56` explains that the recursive glob exists *because* they are served by `/<path:filename>` and nest two levels deep (`preview-r4/v2/`); `GET /preview-next/index.html` returns **200**. Shipped cost is **660 KB = 33.5%** of the webapp's shipped assets. **My earlier row in this table said 4.07 MB / 76% — that was wrong**: it counted 42 PNG screenshots which `pyproject.toml` excludes on purpose. Left in place per the owner's recorded call (`SESSION_SUMMARY.md`: *"leave ... delete = only destructive option"*), and the stale *"not shipped surface"* claim that nearly caused all four to be excluded is corrected in `CRITICAL_REVIEW_2026-09-18.md` with a dated block |
| FE-L4 | LOW | Theme escalation: ~69 `!important`, ~58 `z-index`, two `:root` blocks, dawn declared twice | **SUBSTANCE CONFIRMED, NUMBERS OFF, ONE SUB-CLAIM REFUTED — and it is planned work.** Measured 2026-09-26: **80** `!important`, **51** `z-index` (audit said ~58 — overstated by 7), **2** `:root` blocks (confirmed, and the second at `style.css:4770` is deliberate and self-documenting: *"their palette was never reachable from tungsten.css, so it lives here"*). The audit's `!important` figure is wrong in the other direction, and checking the revision **at the audit's own date** makes it worse: `git show e3b283f:...style.css` has **82**, so the audit undercounted by 13 — and the debt has gone **flat-to-down** since (82 → 80), not grown. That matters because "~69" reads as a smaller problem than it is, while the redesign plan's pay-down is premised on the real number. **"dawn is declared twice" is refuted**: `grep -E '^body\.dawn *\{'` returns exactly one hit (`style.css:379`), and the theme is a single class toggled once (`app.js:1734` `document.body.classList.toggle("dawn", …)`). `REDESIGN_MASTER_PLAN.md` §2 owns the pay-down, so this stays a note rather than a task |
...
**The four that were open are now dispositioned** (2026-09-26, this pass), and only one
of them was a defect of the kind the audit claimed:

- **FE-M2** — the stale comment was real and is fixed, with the union now declared on a
  machine-readable line and asserted against the source. The `"fv"` half was wrong.
- **FE-M3** — **not a defect.** The hiding is deliberate and documented eleven days
  before the audit was written; the audit verified the code and not the comment three
  lines above it. The real defect was the *copy* pointing at the hidden control, which
  the audit did not find. Fixed.
- **FE-L3** — the 26-file count is exact; the word "abandoned" is not. `preview-next/`
  is a live surface with five routes and 14 tests behind it. All four ship on purpose.
  Shipped cost 660 KB, not the 4.07 MB I first measured (that counted excluded PNGs).
- **FE-L4** — substance holds, the numbers are off in both directions, and "dawn is
  declared twice" is refuted outright.

**FE-L1 and FE-L2 are now settled too** (2026-09-26), and they were the audit's
`⚠️ reported` tier — the two it never verified. Both turned out to be wrong:
FE-L1's item-level claim is refuted by measurement (3/3 items are named) and the real
gap one level up is fixed; FE-L2 describes a container retired in Phase 13, and at
900 px the premise view is 100% of the viewport with zero overlap. **Nothing in this
audit is left unmeasured now.** What remains are FE-M4 and FE-M5, which are design
judgements rather than defects, and which need a product decision rather than a
measurement. Everything else across all five audits is closed.
