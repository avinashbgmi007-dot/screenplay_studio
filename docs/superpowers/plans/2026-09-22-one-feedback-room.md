...
- [x] **Step 3:** Spec acceptance walk — every §11 box now carries, inline, the gate that pins it. **One box was reported NOT met rather than ticked:** `app.js` net-negative (Deviations 3). The writer subsequently ruled on it, and §11 box 2 is now amended and ticked.
...
3. **§11 box 2 was NOT met as written, and is now amended.** `app.js` is +985/−410 against `main`, not net-negative: each deletion bought a surface the same spec asks for (collapsible persisted sections, the one-rendering dedupe, the convergence line, the stage ladder, `/quickcheck`, the rule popover). The half that is about the retired surfaces is met and now pinned in three files (JS, markup and sheet — the 43 dead `.fv-*`/`.pb-*` selectors in 193 lines came out in `344c9eb`). Amending the box to "the retired surfaces stay retired" is a product call; the writer gave the go-ahead on 2026-09-23, the box is retitled and ticked, and the line-count clause is struck with the reason next to it.
