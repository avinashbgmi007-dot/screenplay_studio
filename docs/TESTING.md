...
| Error contract & destructive paths | `test_analyze_preflight.py` (the analyze pre-flight — the heartbeat is **written**, not deleted, and a failure there returns JSON), `test_destructive_paths.py` (the `_error` + `@app.errorhandler(Exception)` backstop, and the guarded `os.remove` sites including the `finally` that used to swallow the real error), `test_delete_retry.py`, `test_connection_modes.py` |
