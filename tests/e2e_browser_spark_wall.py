"""e2e_browser_spark_wall.py — cross-check that the desk surfaces are FULLY
live in the app, not a stale cache and not a mockup-only patch.

Phase 9 update: the Spark Wall DECOR (starfield/spark-threads ambience) is
retired per the master migration plan — the idea page is writing-first.
This suite now asserts the decor is GONE while the functional mounts
(#flow-btn, .dawn-wash, #river-current) survive.
...
def main():
    ...
    with open_studio() as base, sync_playwright() as pw:
        ...
        c.check("boot DOM carries the functional mounts (flow/dawn/river)", all(boot.values()),
                json.dumps(boot))
        decor = page.evaluate("""() => ({
        ...
        c.check("Phase 9: starfield/threads decor is absent from the DOM",
                not any(decor.values()), json.dumps(decor))
        ...
        wall = page.evaluate("""() => {
            ...
            return {
                ...
                decorGone: !document.querySelector('.spark-ambience') && !document.querySelector('.spark-threads'),
                graduate: grad ? grad.textContent.trim() : '',
                pill: !!document.getElementById('idea-sam-pill'),
            };
        }""")
        c.check("idea page opens (canvas flex)", wall["open"])
        c.check("Phase 9: the idea page carries no ambience layer", wall["decorGone"])
        c.check("graduate reads 'Grow into pages'", "Grow into pages" in wall["graduate"],
                wall["graduate"])
        c.check("Sameer pill rides the idea page", wall["pill"])
        ...
        # Phase 3 retired the legacy toolbar row (body:not(.idea-mode) .desk)
        # — #flow-btn still lives there until Phase 13 toolbar parity. Its
        # handler, the river CSS (.scene-page rules), and #river-current all
        # target the MIGRATED manuscript surface, so the contract is the
        # handler: invoke it directly (a JS click fires on hidden elements).
        ...
        river = page.evaluate("""() => {
            ...
            const sp = document.querySelector('#manuscript-container .scene-page');
        ...
        scrolled = page.evaluate("""() => {
            const sc = document.getElementById('manuscript-container');
            const dots = [...document.querySelectorAll('#river-current i')];
