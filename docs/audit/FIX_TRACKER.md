...
|---|---|---|
| Unit + integration | `python -m pytest tests/` | **1835 passed, 3 skipped, 0 failed** — measured 2026-09-26 at **`613627e`** (228s; coverage **87%** — 9775 statements, 1262 missed, against the `fail_under = 85` floor; the 3 skips are `test_store_fault_injection`'s structurally-inapplicable non-load-modify-write cases; **+8** over the previous row: `test_spa_contract.py` 8 — the FE-M2 union, the FE-M3 hidden-control contract and the copy rule). The previous figure (1827) was measured at `dbeade7`; it was first recorded against `d19fbce`, which was wrong — the +2 BE-5 tests are in `dbeade7` |
| Lint | `ruff check .` | **clean** (re-measured 2026-09-25) |
| JS unit | `node --test tests/js/*.test.js` | **16 / 16** (re-measured 2026-09-25) |
| Browser E2E | `python tests/run_browser_suites.py` | **55 suites: 54 pass, 0 fail, 1 skip, 0 known-broken** — **1,343 checks** — measured 2026-09-26 at `613627e`, **UNCHANGED from the previous row**. That is the informative part: this revision edits `app.js`, which all 55 suites load, so an unchanged fleet is the evidence that a comment rewrite and two copy strings moved nothing else. `gun_pen_audit` remains the one skip — it POSTs a real `/analyze` and needs a live llama-server. |
...
**Still genuinely unverified:** FE-M4 and FE-M5 are design judgements, not defects, and
FE-L1/FE-L2 need a browser to settle. Everything else across all five audits is closed.
