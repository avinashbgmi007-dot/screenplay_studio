...
| FE-L4 | LOW | Theme escalation: ~69 `!important`, ~58 `z-index`, two `:root` blocks, dawn declared twice | **SUBSTANCE CONFIRMED, NUMBERS OFF, ONE SUB-CLAIM REFUTED — and it is planned work.** Measured 2026-09-26: **80** `!important` (audit said ~69 — understated by 11), **51** `z-index` (said ~58 — overstated by 7), **2** `:root` blocks (confirmed, and the second at `style.css:4770` is deliberate and self-documenting: *"their palette was never reachable from tungsten.css, so it lives here"*). **"dawn is declared twice" is refuted**: `grep -E '^body\.dawn *\{'` returns exactly one hit (`style.css:379`), and the theme is a single class toggled once (`app.js:1734` `document.body.classList.toggle("dawn", …)`). `REDESIGN_MASTER_PLAN.md` §2 owns the pay-down, so this stays a note rather than a task |
