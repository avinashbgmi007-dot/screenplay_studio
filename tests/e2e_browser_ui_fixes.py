...
def run(base):
    ...
    with sync_playwright() as p:
        ...
        # Phase 13: the manuscript renders into #manuscript-container (the
        # legacy #script-scenes container is removed from the DOM)
        expect_visible = page.wait_for_selector("#manuscript-container .scene-page", timeout=25000)
        st2 = page.evaluate("""() => ({
        ...
        # margin-note input keeps its row width — Phase 13: the dock's note
        # form is #dock-note-input now (legacy #rail-note-input retired),
        # and it lives in the Context Dock's Notes lens — open the dock
        # first or the input is hidden (0x0) with the dock closed
        page.evaluate("() => openDock('notes')")
        page.wait_for_timeout(500)
        rn = page.evaluate("""() => {
            const i = document.querySelector('#dock-note-input');
            if (!i) return null;
        ...
        page.keyboard.press("Escape")   # dock away — back to the manuscript
        page.wait_for_timeout(400)
